...
//	private JSONObject machineRateInfo = null;
	...
	public  void getText(InformactionModel im){
			...
			for (int i = 0; i < errorResult.size(); i++) {
				...
				for (int j = 0; j < errorList.size(); j++) {
					...
					.set("error", getError(error.getString("typeName"))).set("sentence_location", Error.getString("index")).set("error_word", geTerrorWord(error,Error))
	...
	private int getError(String error) {
		if("主语缺失".equals(error)){
			return 146;
		}
		if("主谓一致错误".equals(error)){
			return 147;
		}
		if("介词短语错误".equals(error)){
			return 148;
		}
		if("代词格错误".equals(error)){
			return 149;
		}
		if("关联词多余".equals(error)){
			return 150;
		}
		if("其它错误".equals(error)){
			return 151;
		}
		if("冠词错误".equals(error)){
			return 152;
		}
		if("动词及物-不及物".equals(error)){
			return 153;
		}
		if("动词时态错误".equals(error)){
			return 154;
		}
		if("动词短语错误".equals(error)){
			return 155;
		}
		if("动词语态错误".equals(error)){
			return 156;
		}
		if("助动词缺失".equals(error)){
			return 157;
		}
		if("名词单复数错误".equals(error)){
			return 158;
		}
		if("名词可数-不可数".equals(error)){
			return 159;
		}
		if("存现句[there_be]错误".equals(error)){
			return 160;
		}
		if("定语从句关系词错误".equals(error)){
			return 161;
		}
		if("并列关系错误".equals(error)){
			return 162;
		}
		if("形容词短语错误".equals(error)){
			return 163;
		}
		if("惯用语错误".equals(error)){
			return 164;
		}
		if("拼写错误".equals(error)){
			return 165;
		}
		if("无主句".equals(error)){
			return 166;
		...
		if("标点错误".equals(error)){
			return 169;
		}
		if("流水句".equals(error)){
			return 170;
		}
		if("词类混用错误".equals(error)){
			return 171;
		}
		if("语序错误".equals(error)){
			return 172;
		}
		if("谓语动词缺失".equals(error)){
			return 173;
		}
		if("连词缺失".equals(error)){
			return 174;
		}
		if("非谓语动词错误".equals(error)){
			return 175;
		}
